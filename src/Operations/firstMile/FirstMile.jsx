...
const FirstMile = ({ isSidebarAdmin }) => {
  ...
  const isOperationRoute = location.pathname === "/adminDashboard/operations/firstmile";
  const tabStorageKey = isOperationRoute ? "activeOperationTab" : "activeOrderTab";
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "Booked":
        return <BookedOrders orders={filteredOrders} userId={userId} />;
      case "Not Picked":
        return <NotPickedOrders orders={filteredOrders} userId={userId} />;
      case "Ready To Ship":
        return <ReadyToShipOrders orders={filteredOrders} userId={userId} />;
      default:
  ...
  return (
    <div className={`md:px-2 px-1 ${showModal ? "overflow-hidden" : ""}`}>
      <div className="mb-1">
        <h1 className="text-[12px] md:text-[18px] font-[600] text-gray-700">First Mile</h1>
      </div>
