...
const ReadyToShipOrders = (filterOrder, { isSidebarAdmin }) => {
  ...
  // console.log("ooooooooooooo",filterOrder.orders)
  ...
  const handleInvoice = async (id) => {
    try {
      ...
      Notification("Invoice download successfully", "success")
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
    ...
    try {
      ...
      const orderResponses = await Promise.all(
        selectedOrders.map((id) =>
          fetch(`${REACT_APP_BACKEND_URL}/orders/checkCourier/${id}`).then((res) => res.json())
        )
      );
      ...
      for (const orderData of orderResponses) {
        ...
        for (const page of copiedPages) {
          if (selectedOrders.length === 1) {
            ...
            const y =
              pageHeight -
              ((Math.floor(labelCount / 2) % 2) + 1) * labelHeight;
                ...
                <div className="flex w-full md:w-1/3 max-w-md">
                  <div className="relative w-3/5" ref={awbDropdownRefMobile}>
                    <button
                      ...
                      className="w-full h-8 px-3 text-[12px] font-[600] border-2 border-r-0 rounded-l-lg bg-white text-left focus:outline-none flex items-center justify-between text-gray-500"
                    >
                      <span className="truncate">
                        {options.find((opt) => opt.value === searchBy)?.label ||
                          "Select"}
