...
const Undelivered = (filterData, { isSidebarAdmin }) => {
  ...
  const [paymentType, setPaymentType] = useState(""); // Add this
  ...
  const [bulkNdrOpen, setBulkNdrOpen] = useState(false);
  ...
  const getAttemptSuffix = (attempt) => {
    if (attempt === 1) return "st";
    if (attempt === 2) return "nd";
    if (attempt === 3) return "rd";
    return "th"; // Default case
  };
  ...
  const handleViewHistory = (ndrHistory) => {
    setSelectedNdrHistory(ndrHistory); // Set the order's ndrHistory
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${REACT_APP_BACKEND_URL}/admin/filterNdrOrdersForEmployee`,
        {
          params,
          headers: {
  ...
  const handleNdrAction = async (payload) => {
    ...
    try {
      const response = await axios.post(
        `${REACT_APP_BACKEND_URL}/ndr/ndr-process/`,
        payload
      );
      ...
      if (response.data.success) {
        ...
        Notification(response.data.data, "error");
      }
    } catch (error) {
      Notification("Failed to process NDR action. Please try again.", "error");
            ...
            <div className="flex items-center justify-between gap-2 relative">
              <div className="w-[210px] flex-1 relative">
                <input
                  type="text"
                  placeholder="Search by Name, Email, or Contact"
                  className="w-full h-8 px-3 placeholder:text-[12px] text-[12px] font-[600] border-2 rounded-lg placeholder:font-[600] placeholder:text-gray-400 focus:outline-none"
                  onChange={(e) => {
                    const value = e.target.value;
                    setSearchUser(value);
                    if (value.trim() === "") {
              ...
              {/* FaFilter Button */}
              <button
                className="px-3 flex items-center border-2 border-gray-100 justify-center text-white bg-[#0CBB7D] h-8 rounded-lg transition text-[12px] font-[600]"
          ...
          <tbody>
            {loading1 ? (
              ...
              orders.map((order, index) => (
                <tr
                  key={index}
                  className="border text-gray-500 hover:bg-gray-50 transition-all text-[12px] font-[500]"
                >
                  ...
                  <td className="py-2 px-3 whitespace-nowrap" style={{ maxWidth: "170px", width: "120px" }}>
                  ...
                  <td className="py-2 px-3 whitespace-nowrap leading-5">
                    {/* Latest NDR date */}
                    <p className="text-[10px]">
                      {order?.ndrReason?.date
                        ? (() => {
                          const d = new Date(new Date(order.ndrReason.date).getTime() - 5.5 * 3600 * 1000);
                    ...
                    <div className="flex gap-2">
                      <p className="cursor-pointer relative group">
                        {order.ndrHistory?.length
                          ? `${order.ndrHistory.length}${getAttemptSuffix(
                            order.ndrHistory.length
                          )} Attempt`
                          : "No Attempts"}
                        <i className="fas fa-info-circle text-[#0CBB7D] ml-2"></i>

                        {/* Tooltip (Address Popup) */}
                        <div
                          className="absolute z-50 invisible opacity-0 group-hover:visible group-hover:opacity-100 
transition-opacity duration-200 bg-white border border-gray-200 text-[11px] 
text-gray-700 p-3 rounded-md shadow-xl max-w-xs max-h-56 w-[250px] overflow-y-auto top-1/2 left-[-200px] ml-2 transform -translate-y-1/2"
                          style={{
                            pointerEvents: "none", // prevent stealing focus
                          }}
                        >
                          ...
                          {/* Address Information */}
                          <p className="font-semibold">Last Ndr Reason:</p>
                          <p className="break-words text-[8px] font-semibold">
                            {order?.ndrReason?.reason}
                          </p>
                          {/* <p className="text-[10px]">{order.receiverAddress.state}, {order.receiverAddress.city} ,{order.receiverAddress.pinCode}</p> */}
                        </div>
                      </p>
                    </div>
                    <span
                      className="text-white text-[10px] rounded-lg bg-[#0CBB7D] px-2 py-[5px] cursor-pointer"
                      onClick={() => handleViewHistory(order.ndrHistory)}
                    >
                      View History
                    </span>
                  </td>
                  ...
                  {/* <td className="py-3 px-4 whitespace-nowrap">
                    <button
                      className="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg text-xs md:text-sm"
                      onClick={() => {
                        if (
                          isSidebarAdmin ||
                      ...
                      <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                        <ul className="py-2">
                          ...
                          >
                            <button
                              onClick={() =>
                                handleNdrAction(order.awb_number, "RE-ATTEMPT")
                              }
                              disabled={loading}
                            >
                              {loading ? "Processing..." : "Re-attempt"}
                            </button>
                          </li>

                          {order.provider !== "Delhivery" && (
                            <li className="px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer">
                              <button
                              //  onClick={() => handleLabel(order._id)}
                              >
                                Update Address
                              </button>
                            </li>
                  ...
                  {/* <td className="py-2 px-3 whitespace-nowrap relative">
                    {(order.provider === "DTDC" || order.provider === "Dtdc") ? (
                      <button
                        className={`px-3 py-2 rounded-lg text-[10px] font-[600] text-white 
                        ${order?.reattempt ? "bg-[#0CBB7D]" : "bg-gray-400 cursor-not-allowed"}`}
                        onClick={order?.reattempt ? () => openModal(order) : undefined}
                        disabled={!order?.reattempt} // ✅ button disabled if reattempt is false
                      >
                        Take Action
                      </button>
                    ) : (
                      <>
                        <button
                          className="px-3 py-2 bg-[#0CBB7D] text-white rounded-lg text-[10px] md:text-[10px] font-[600]"
                    ...
                    {selectedOrder && selectedOrder._id === order._id && order.provider === "DTDC" && order?.reattempt && (
                      <NdrActionModal
                        isOpen={modalOpen}
                        onClose={closeModal}
                        order={selectedOrder}
              ...
              <tr>
                <td colSpan="12" className="text-gray-500 text-center py-6">
      ...
      <div className="md:hidden space-y-4">
        ...
        {loading1 ? (
          ...
          orders.map((order, index) => (
            <div
              key={index}
              className="text-[#2d054b] mt-3 bg-green-50 p-3 rounded-lg border-1 border-[#2d054b] shadow-md"
            >
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  />
                  <span className="flex">
                    <p className="font-[600] mt-1 text-[10px]">Order ID: </p>

                    <Link
                      to={`/dashboard/order/neworder/updateOrder/${order._id}`}
                      className="text-[#0CBB7D] font-[600] mt-1 text-[10px] cursor-pointer"
                    >
                      {order.orderId}
                    </Link>
                    <p className="ml-2 font-[600] px-2 py-1 text-[10px] bg-green-200 text-green-700 rounded-lg">
                      {order.ndrStatus}
                    </p>
                  </span>
                </div>
                {/* <div>
                  <button
                    className="px-4 py-2 text-gray-700 rounded-lg text-xs md:text-sm"
                    onClick={() => {
                      if (
                        isSidebarAdmin ||
                        employeeAccess.isAdmin ||
                  ...
                  >
                    <HiDotsHorizontal size={20} className="text-[#2d054b]" />
                  </button>
                  {dropdownOpen === index && (
                    <div className="absolute right-0 mt-2 w-32 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                      <ul className="py-2">
                        <li
                          className={`px-4 py-2 text-gray-700 hover:bg-gray-100 cursor-pointer ${loading ? "cursor-not-allowed" : ""
                            }`}
                        >
                          <button
                            onClick={() =>
              ...
              <div className="flex items-center justify-between bg-green-200 p-3 rounded-lg shadow-md gap-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <p className="text-[10px] font-[600] text-gray-600">
                    ...
                    <p className="text-[10px] text-gray-500">
                      ...
                      <span
                        onClick={() => handleTrackingByAwb(order.awb_number)}
                        className="text-green-500 text-[10px] cursor-pointer font-[600]"
                      >
                        {order.awb_number}
                      </span>
                    </p>
                ...
                <div className="text-right">
                  <p className="ml-2 font-[600] px-2 py-1 rounded-lg text-[10px] bg-green-100 text-green-700">
                    {order.status}
                  </p>
                </div>
              </div>
              ...
              <div className="p-2 bg-gray-200 text-[10px] rounded-lg mt-2">
                <p className="font-[600]">Last NDR Reason:</p>
                <p className="max-w-[170px] break-words truncate text-[10px]">
                  {order?.ndrReason?.reason || "Consignee Unavailable"}
                </p>
              </div>
              ...
              <div className="mt-1 flex gap-2">
                <div className="flex items-center justify-between gap-1 w-full">
                  ...
                  <div>
                    ...
                    <p className="text-[10px]">
                      {order.ndrHistory?.[order.ndrHistory.length - 1]?.date
                        ? (() => {
                          const rawDate = order.ndrHistory[order.ndrHistory.length - 1].date;
                          const d = new Date(new Date(rawDate).getTime() + 5.5 * 3600 * 1000);
                          ...
                          return `${day} ${month} ${year} ${hourStr}:${minutes}:${seconds} ${period}`;
                        })()
                        : "N/A"}
                    </p>
                  ...
                  {/* View History */}
                  <span
                    className="text-green-600 cursor-pointer underline text-[10px] md:ml-4 mt-1 md:mt-0"
                    onClick={() => handleViewHistory(order.ndrHistory)}
                  >
                    View History
                  </span>
          ...
          <div className="text-center py-6 text-gray-500"><div className="flex flex-col items-center justify-center">
            <img
              src={NotFound}
              alt="No Data Found"
              className="w-60 h-60 object-contain mb-2"
