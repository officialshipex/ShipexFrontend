...
const DelayDelivered = (filterOrder, { isSidebarAdmin }) => {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            const response = await axios.get(
                `${REACT_APP_BACKEND_URL}/admin/filterDelayDeliveredOrders`,
                {
                    params,
                    headers: {
    ...
    return (
        <div className="px-2">
            <div className="mb-1">
                <h1 className="text-[12px] md:text-[18px] font-[600] text-gray-700">Delay Delivered</h1>
            </div>
            {/* Desktop Table */}
            <div>
                ...
                <table className="min-w-full bg-white border border-gray-300 rounded-lg shadow-md">
                  ...
                  text-gray-700 p-3 rounded-md shadow-xl overflow-auto w-[300px] top-1/2 left-[20px] ml-2 transform -translate-y-1/2"
                                    ...
                                    <td className="py-2 px-3 whitespace-nowrap" style={{ maxWidth: "150px", width: "100px" }}>
                                        ...
                                        <p className="text-[10px]">
                                            Delivered on:{" "}
                                            {order.tracking && order.tracking.length && order.tracking.at(-1).StatusDateTime
                                                ? new Date(new Date(order.tracking.at(-1).StatusDateTime).getTime() - 5.5 * 60 * 60 * 1000).toLocaleDateString()
                                                : "-"}
                                        </p>
                                    </td>
