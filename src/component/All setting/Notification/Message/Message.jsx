...
const SmsNotification = () => {
  ...
  // Status list with templates
  const statuses = [
    {
      key: "PickupPending",
      label: "Pickup Pending",
      template:
        "Your order has been created and is pending pickup. We’ll notify you once it’s picked up.",
    },
    {
      key: "Intransit",
      label: "In Transit",
      template: "Your order is on the way and currently in transit.",
    },
    {
      key: "OutForDelivery",
      label: "Out for Delivery",
      template: "Your order is out for delivery. Please be available to receive it.",
    },
    {
      key: "Delivered",
      label: "Delivered",
      template: "Your order has been successfully delivered. Thank you for shopping with us!",
    },
    {
      key: "Undelivered",
      label: "Undelivered",
      template: "Delivery attempt was unsuccessful. We will retry soon.",
    },
    {
      key: "RTO",
      label: "RTO Initiated",
      template: "Your order is being returned to the sender (RTO initiated).",
    },
  ];
  ...
  const handleToggle = async (key) => {
    ...
    try {
      ...
      const res = await axios.put(
        ...
        { field: `isSMS${key}Enable`, value: newValue },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleMainToggle = async (value) => {
    ...
    try {
      ...
      await axios.put(
        `${REACT_APP_BACKEND_URL}/notification/updateNotification`,
        { field: "isUserSMSEnable", value },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      Notification("SMS Notification update successfully","success"
      );
    } catch (error) {
      Notification("Error Updating SMS Notification", "error");
