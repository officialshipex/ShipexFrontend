...
const AllDiscrepancy = () => {
  ...
  const fetchDiscrepancy = async () => {
    try {
      ...
      const params = {
        ...
        limit,
        provider,
        status // status can be empty for "all"
      };
  ...
  useEffect(() => {
    fetchDiscrepancy();
  }, [dateRange, page, limit, inputValue, searchBy, provider, status]);
            ...
            orders.map((order, index) => (
              <div key={index} className="bg-white rounded-lg border border-gray-300 overflow-hidden text-[12px] relative">
                ...
                <div className="flex justify-between items-center px-3 py-2 bg-green-100">
                  ...
                  <span className="ftext-gray-500">AWB : <span className="text-[#0CBB7D]">{order.awbNumber}</span></span>
