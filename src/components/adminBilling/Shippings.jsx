        ...
        <div className="h-[calc(100vh-235px)] text-[12px] overflow-y-auto bg-white">
          <table className="w-full border-collapse">
            <thead className="bg-brand-primary text-white font-[600] sticky top-0 z-10">
              <tr className="bg-brand-primary text-white font-[600] text-left">
              ...
              {loading ? (
                ...
                transactions.map((row, index) => (
                  <tr key={index} className="border-b border-gray-300 font-[400] text-[12px] hover:bg-gray-50">
                    ...
                    <td className="py-2 px-3">
                      <p className="text-brand-primary font-medium">{row.user?.userId}</p>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1 group">
                        <Link to={`/dashboard/order/neworder/updateOrder/${row._id}`} className="text-brand-primary font-medium">
                          {row.orderId}
                        </Link>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1 group">
                        <p className="text-brand-primary cursor-pointer" onClick={() => handleTrackingByAwb(row.awb_number)}>
                    ...
                    <td className="py-2 px-3 font-medium">
                      <div className="flex items-center gap-1">
                        ...
                        {(row.totalFreightCharges || row.weightDiscrepancy?.excessWeightCharges?.excessCharges) && (
                          ...
                          >
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-brand-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" strokeWidth="2" /><path d="M12 8v4m0 4h.01" strokeWidth="2" strokeLinecap="round" /></svg>
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="py-2 px-3 text-brand-primary">
                      <button
                        onClick={() => navigate("/finance/billing/passbook", { state: { awbNumber: row.awb_number } })}
                        className="bg-brand-primary text-white px-3 py-1 rounded-md hover:bg-opacity-90 transition-all font-[600] text-[12px]"
                      >
                        History
                      </button>
                    </td>
        ...
        <div className="flex items-center justify-between gap-2 bg-white p-2 rounded-lg shadow-sm border border-gray-100">
          ...
          <div className="relative" ref={actionRef}>
            <button
              disabled={selectedOrders.length === 0}
              onClick={() => setActionOpen(!actionOpen)}
              className={`h-[30px] px-3 rounded-lg flex items-center justify-center border transition-all ${selectedOrders.length > 0 ? "border-brand-primary text-brand-primary bg-white shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                }`}
            >
              ...
              <div key={row._id} className="bg-white rounded-lg shadow-sm px-3 py-2 border border-gray-200 animate-popup-in relative">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-gray-700 font-[600] text-[10px]">Order ID : <span className="text-brand-primary font-[600]">{row.orderId}</span></span>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => navigate("/finance/billing/passbook", { state: { awbNumber: row.awb_number } })}
                      className="text-brand-primary hover:text-[#099e68] transition-all"
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px] mb-2">
                  <div>
                    ...
                    <div className="flex items-center gap-1 group">
                      <p className="text-brand-primary font-bold active:underline" onClick={() => handleTrackingByAwb(row.awb_number)}>
                        {row.awb_number || "N/A"}
                      </p>
                      {row.awb_number && (
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-end gap-1">
                      <p className="text-brand-primary font-bold">₹{row.totalFreightCharges || 0}</p>
                      {row.totalFreightCharges && (
                        <button
                          ...
                          className="text-brand-primary cursor-pointer flex-shrink-0 p-1.5 -m-1.5"
                        >
                      ...
                      >
                        ...
                        <div className="space-y-2 text-[12px]">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="font-bold text-brand-primary">{row.orderType === "B2C" ? row.packageDetails?.applicableWeight : row.B2BPackageDetails?.applicableWeight} Kg</span>
                          </div>
                          {row.orderType === "B2C" && (
                            <>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full border border-gray-300 flex items-center justify-center text-brand-primary font-bold text-[12px]">
                  ...
                  <div className="flex justify-between items-center w-full min-w-0">
                    ...
                    <p className="text-brand-primary font-[600] text-[10px] truncate">{row.user?.userId}</p>
                  </div>
                </div>
              </div>
            ))
