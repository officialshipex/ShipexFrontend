                    ...
                    <table className="w-full text-[12px] text-left border-collapse relative">
                        <thead className="sticky top-0 z-40 bg-brand-primary text-white font-[600]">
                            <tr>
                                ...
                                <th className="px-3 py-2 shadow-[0_1px_0_0_var(--brand-primary,#0CBB7D)]">Date</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_var(--brand-primary,#0CBB7D)]">Transaction ID</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_var(--brand-primary,#0CBB7D)]">Amount</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_var(--brand-primary,#0CBB7D)]">Status</th>
                                <th className="px-3 py-2 shadow-[0_1px_0_0_var(--brand-primary,#0CBB7D)]">Description</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={row._id} className="hover:bg-gray-50 text-[12px] transition-colors border-b border-gray-200">
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex flex-col text-gray-700">
                                                <span className="text-brand-primary font-medium leading-tight">{row.user.userId}</span>
                                        ...
                                        <td className="px-3 py-2">
                                            <div className="flex items-center gap-1 group">
                                                <span className="text-brand-primary font-medium">{row.transactionId}</span>
                                                    ...
                                                    <div className="flex items-center gap-2 group">
                                                        ...
                                                        {row.paymentId && (
                                                            <button onClick={() => handleCopy(row.paymentId, row._id + '_pay')}>
                                                                {copiedId === row._id + '_pay' ? <FiCheck className="w-3 h-3 text-brand-primary" /> : <FiCopy className="w-3 h-3 text-gray-300 transition-opacity opacity-0 group-hover:opacity-100" />}
                                                            </button>
                                                        )}
                                                    </div>
                                                </div>
                ...
                <div className="p-2 gap-2 bg-white rounded-lg flex justify-between items-center border border-gray-100 mb-2 shadow-sm">
                    ...
                    <div className="relative" ref={actionRef}>
                        <button
                            disabled={selectedTransactions.length === 0}
                            onClick={() => setActionOpen(!actionOpen)}
                            className={`h-[30px] px-3 rounded-lg flex items-center justify-center border transition-all ${selectedTransactions.length > 0 ? "border-brand-primary text-brand-primary bg-white shadow-sm" : "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
                                }`}
                        >
                            ...
                            <div key={row._id} className="bg-white rounded-lg shadow-sm p-3 border border-gray-200 text-[10px] animate-popup-in">
                                <div className="flex justify-between mb-2 items-start relative">
                                    ...
                                    <div className="pl-6 space-y-1">
                                        <div className="flex items-center gap-1 font-[600] text-gray-700">
                                            <span className="text-brand-primary truncate max-w-[120px] font-bold">{row.transactionId}</span>
                                            <button onClick={() => handleCopy(row.transactionId, row._id + '_txn_mobile')}>
                                                {copiedId === row._id + '_txn_mobile' ? <FiCheck className="w-3 h-3 text-brand-primary" /> : <FiCopy className="w-3 h-3 text-gray-400" />}
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            {row.paymentId && (
                                                <button onClick={() => handleCopy(row.paymentId, row._id + '_pay_mobile')}>
                                                    {copiedId === row._id + '_pay_mobile' ? <FiCheck className="w-2.5 h-2.5 text-brand-primary" /> : <FiCopy className="w-2.5 h-2.5 text-gray-400" />}
                                                </button>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="mt-2 pt-2 border-t border-gray-50 flex items-center justify-between px-1">
                                    <div className="flex items-center gap-2 min-w-0">
                                        <div className="w-6 h-6 rounded-full bg-white flex items-center justify-center font-bold text-brand-primary text-[10px] shrink-0 border border-gray-300">
                                    ...
                                    <p className="text-brand-primary font-medium text-[10px] tracking-widest shrink-0">
                                        {row.user.userId}
                                    </p>
                                </div>
                            </div>
