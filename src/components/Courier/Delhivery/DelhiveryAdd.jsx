...
const DelhiveryAdd = ({ onCourierSaved, canAction }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await axios.post(`${REACT_APP_BACKEND_URL}/Delhivery/getToken`, newCourier);
      onCourierSaved?.();
  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:flex gap-8">
        <div>
          <label className="font-[600] block text-[12px] text-gray-500">Courier Name</label>
          <input
            type="text"
            className="w-72 px-3 text-[12px] text-gray-500 h-9 border focus:outline-none border-gray-300 rounded-lg"
            value={courierName}
            onChange={(e) => setCourierName(e.target.value)}
          />
        </div>
        <div>
          <label className="font-[600] block text-[12px] text-gray-500">COD Days</label>
          <input
            type="number"
            className="w-32 px-3 text-[12px] text-gray-500 h-9 border border-gray-300 rounded-lg focus:outline-none"
            value={codDays}
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-8">
        <div className="w-full md:w-80">
          ...
          <input
            type="email"
            className="w-full px-3 py-2 text-[12px] text-gray-500 border border-gray-300 rounded focus:outline-none"
