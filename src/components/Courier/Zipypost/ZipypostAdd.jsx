...
const Zipypost = ({ canAction }) => {
    ...
    const handleSave = async () => {
        try {
            const newCourier = {
                courierName,
                courierProvider: "ZipyPost",
                CODDays: codDays,
                status: Status,
                credentials: { username, password },
            };

            const response = await axios.post(`${REACT_APP_BACKEND_URL}/ZipyPost/authorize`, newCourier);
            console.log(response.data);

            Notification('Courier added successfully', "success");
            navigate("/dashboard/setup/courier");
        } catch (error) {
            console.error("Error validating Shiprocket credentials:", error);
    ...
    return (
        <div className="w-full">
            ...
            {/* Credentials Section (Both Views) */}
            <div className="flex flex-col md:flex-row gap-4 md:gap-8">
                <div className="w-full md:w-auto">
                    <label className="font-[600] block text-[10px] sm:text-[12px] text-gray-500">Username <span className='text-red-600'>*</span></label>
                    <input
                        type="email"
                        className="w-full md:w-72 px-3 text-[12px] text-gray-500 h-9 border border-gray-300 rounded-lg focus:outline-none"
                ...
                <div className="w-full md:w-auto">
                    <label className="font-[600] block text-[10px] sm:text-[12px] text-gray-500">Password <span className='text-red-600'>*</span></label>
                    <input
                        type="password"
                        className="w-full md:w-72 px-3 text-[12px] text-gray-500 h-9 border border-gray-300 rounded-lg focus:outline-none"
